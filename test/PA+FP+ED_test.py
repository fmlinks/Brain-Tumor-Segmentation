...
class FPN0:
    ...
    def train(self):
        ...
        checkpointer = ModelCheckpoint(filepath='../weights/PA+FP+ED/0/PAUnet.{epoch:02d}.hdf5',
        ...
        with open('../weights/PA+FP+ED/0/dunet_tfcv5.json', 'w') as f:
    ...
    def test(self):
        model = self.get_model()
        model.summary()
        model.load_weights('../weights/PA+FP+ED/0/PAUnet.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        ...
        print(pred1.shape)
        np.save('../results/PA+FP+ED/0/paunet_tfcv5.npy', pred1)
...
class FPN1:
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/PA+FP+ED/1/PAUnet.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)

        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
        with open('../weights/PA+FP+ED/1/dunet_tfcv5.json', 'w') as f:
            json.dump(hist.history, f)
    ...
    def test(self):
        model = self.get_model()
        model.summary()
        model.load_weights('../weights/PA+FP+ED/1/PAUnet.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        ...
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/PA+FP+ED/1/paunet_tfcv5.npy', pred1)
...
class FPN2:
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/PA+FP+ED/2/PAUnet.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)

        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
        with open('../weights/PA+FP+ED/2/dunet_tfcv5.json', 'w') as f:
            json.dump(hist.history, f)
    ...
    def test(self):
        model = self.get_model()
        model.summary()
        model.load_weights('../weights/PA+FP+ED/2/PAUnet.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        ...
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/PA+FP+ED/2/paunet_tfcv5.npy', pred1)
...
class FPN3:
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/PA+FP+ED/3/PAUnet.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)

        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
        with open('../weights/PA+FP+ED/3/dunet_tfcv5.json', 'w') as f:
            json.dump(hist.history, f)
    ...
    def test(self):
        model = self.get_model()
        model.summary()
        model.load_weights('../weights/PA+FP+ED/3/PAUnet.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        ...
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/PA+FP+ED/3/paunet_tfcv5.npy', pred1)
...
class FPN4:
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/PA+FP+ED/4/PAUnet.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)

        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
        with open('../weights/PA+FP+ED/4/dunet_tfcv5.json', 'w') as f:
            json.dump(hist.history, f)
    ...
    def test(self):
        model = self.get_model()
        model.summary()
        model.load_weights('../weights/PA+FP+ED/4/PAUnet.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        ...
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/PA+FP+ED/4/paunet_tfcv5.npy', pred1)
