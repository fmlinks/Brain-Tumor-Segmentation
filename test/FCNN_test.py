...
class FCNN0():
    ...
    def train(self):
        ...
        checkpointer = ModelCheckpoint(filepath='../weights/FCNN/0/deconv_150_5.{epoch:02d}.hdf5',
    ...
    def test(self):
        model = self.net((240, 240, 4), dim=32, factor=2)
        model.load_weights('../weights/FCNN/0/deconv_150_5.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        pred = model.predict(X_test, verbose=1)
        # np.save('../results/FCNN/0/fcnn_tfcv5_1hot.npy', pred)
        print(pred.shape)
        print('#### start test ###')
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/FCNN/0/fcnn_tfcv5.npy', pred1)
...
class FCNN1():
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/FCNN/1/deconv_150_5.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)
        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
    ...
    def test(self):
        model = self.net((240, 240, 4), dim=32, factor=2)
        model.load_weights('../weights/FCNN/1/deconv_150_5.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        pred = model.predict(X_test, verbose=1)
        # np.save('../results/FCNN/1/fcnn_tfcv5_1hot.npyy', pred)
        print(pred.shape)
        print('#### start test ###')
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/FCNN/1/fcnn_tfcv5.npy', pred1)
...
class FCNN2():
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/FCNN/2/deconv_150_5.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)
        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
    ...
    def test(self):
        model = self.net((240, 240, 4), dim=32, factor=2)
        model.load_weights('../weights/FCNN/2/deconv_150_5.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        pred = model.predict(X_test, verbose=1)
        # np.save('.../results/FCNN/2/fcnn_tfcv5_1hot.npy', pred)
        print(pred.shape)
        print('#### start test ###')
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/FCNN/2/fcnn_tfcv5.npy', pred1)
...
class FCNN3():
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/FCNN/3/deconv_150_5.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)
        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=70, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
    ...
    def test(self):
        model = self.net((240, 240, 4), dim=32, factor=2)
        model.load_weights('../weights/FCNN/3/deconv_150_5.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        pred = model.predict(X_test, verbose=1)
        # np.save('../results/FCNN/3/fcnn_tfcv5_1hot.npy', pred)
        print(pred.shape)
        print('#### start test ###')
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/FCNN/3/fcnn_tfcv5', pred1)
...
class FCNN4():
    ...
    def train(self):
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy', self.dice_comp,
                                                                                  self.dice_core, self.dice_en])
        checkpointer = ModelCheckpoint(filepath='../weights/FCNN/5/deconv_150_5.{epoch:02d}.hdf5',
                                       monitor='val_loss', verbose=1)
        hist = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=2, batch_size=16,
                         shuffle=True, callbacks=[checkpointer, change_lr])
        with open('../weights/FCNN/5/fcnn_tfcv5.json', 'w') as f:
            json.dump(hist.history, f)


    def test(self):
        model = self.net((240, 240, 4), dim=32, factor=2)
        model.load_weights('../weights/FCNN/4/deconv_150_5.70.hdf5')
        X_train, Y_train, X_test, Y_test = self.load_data('../data/', mode=False)
        del X_train, Y_train, Y_test
        model.summary()
        print(X_test.shape)
        pred = model.predict(X_test, verbose=1)
        # np.save('../results/FCNN/4/fcnn_tfcv5_1hot.npy', pred)
        print(pred.shape)
        print('#### start test ###')
        pred1 = np.argmax(pred, axis=3)
        print(pred1.shape)
        np.save('../results/FCNN/4/fcnn_tfcv5.npy', pred1)
